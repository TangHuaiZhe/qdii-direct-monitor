...
function fundTableRow(code, rows, fee) {
  ...
  return `<tr class="fund-row" data-search="${escapeHtml(search)}" data-share="${escapeHtml(fund.shareClass || "其他")}" data-status="${escapeHtml(status)}" data-purchasable="${best ? "1" : "0"}" data-amount="${bestAmount}" data-agency-amount="${sortableChannelAmount(agency)}" data-direct-amount="${sortableChannelAmount(direct)}" data-fee="${Number.isFinite(fee?.annualRate) ? fee.annualRate : ""}"><td class="fund-cell"><span class="index-chip manager-tone-${mark.tone}" aria-hidden="true">${escapeHtml(mark.initial)}</span><div><strong><a class="fund-link" href="funds/${escapeHtml(code)}/">${escapeHtml(fund.fundName)}</a></strong><span>${escapeHtml(code)} · ${escapeHtml(fund.manager)} · ${escapeHtml(fund.shareClass || "其他")} 类</span></div></td>${channelCell(agency, salesDetailRow(rows, "agency"))}${channelCell(direct, salesDetailRow(rows, "direct"))}${feeCell(fee)}</tr>`;
}
...
(() => {
  ...
  const body = document.getElementById("fund-body"); const rows = [...body.querySelectorAll(".fund-row")]; const search = document.getElementById("fund-search"); const purchasable = document.getElementById("purchasable-only"); const sort = document.getElementById("fund-sort"); const count = document.getElementById("visible-count"); const empty = document.getElementById("empty"); const columnButtons = [...document.querySelectorAll("[data-column-sort]")]; let share = "全部"; let columnSort = null;
