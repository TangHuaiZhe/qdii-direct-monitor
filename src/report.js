...
(() => {
  ...
  const body = document.getElementById("fund-body"); const rows = [...body.querySelectorAll(".fund-row")]; const search = document.getElementById("fund-search"); const purchasable = document.getElementById("purchasable-only"); const sort = document.getElementById("fund-sort"); const count = document.getElementById("visible-count"); const empty = document.getElementById("empty"); const columnButtons = [...document.querySelectorAll("[data-column-sort]")]; const resetSort = document.getElementById("reset-table-sort"); let share = "全部"; let columnSort = null;
  ...
  columnButtons.forEach((button) => button.addEventListener("click", () => { const key = button.dataset.columnSort; columnSort = columnSort?.key === key ? { key, direction: columnSort.direction * -1 } : { key, direction: 1 }; updateColumnButtons(); refresh(); }));
  resetSort.addEventListener("click", () => { columnSort = null; sort.value = "amount"; updateColumnButtons(); refresh(); });
