...
function strategyLabel(strategy) {
  return { active: "主动", passive: "被动", unknown: "未分类" }[strategy] || "未分类";
}
...
function channelCell(row, detailRow) {
...
function sortableChannelAmount(row) {
  if (!row || !["open", "limited"].includes(row.status)) return "";
  return row.status === "open" ? Number.MAX_SAFE_INTEGER : row.limitAmount;
}

function managerMark(manager) {
  const name = String(manager || "");
  const initial = Array.from(name).find((character) => /[\u3400-\u9fff]/.test(character)) || Array.from(name)[0] || "基";
  const tone = Array.from(name).reduce((hash, character) => (hash * 31 + character.codePointAt(0)) >>> 0, 0) % 6;
  return { initial, tone };
}
...
function fundTableRow(code, rows, fee) {
  ...
  const mark = managerMark(fund.manager);
  const strategy = fund.strategy || "unknown";
  return `<tr class="fund-row" data-search="${escapeHtml(search)}" data-share="${escapeHtml(fund.shareClass || "其他")}" data-strategy="${escapeHtml(strategy)}" data-status="${escapeHtml(status)}" data-purchasable="${best ? "1" : "0"}" data-amount="${bestAmount}" data-agency-amount="${sortableChannelAmount(agency)}" data-direct-amount="${sortableChannelAmount(direct)}" data-fee="${Number.isFinite(fee?.annualRate) ? fee.annualRate : ""}"><td class="fund-cell"><span class="index-chip manager-tone-${mark.tone}" aria-hidden="true">${escapeHtml(mark.initial)}</span><div><strong><a class="fund-link" href="funds/${escapeHtml(code)}/">${escapeHtml(fund.fundName)}</a></strong><span class="fund-meta">${escapeHtml(code)} · ${escapeHtml(fund.manager)} · <b class="strategy-badge strategy-${escapeHtml(strategy)}">${escapeHtml(strategyLabel(strategy))}</b> · ${escapeHtml(fund.shareClass || "其他")} 类</span></div></td>${channelCell(agency, salesDetailRow(rows, "agency"))}${channelCell(direct, salesDetailRow(rows, "direct"))}${feeCell(fee)}</tr>`;
}
...
function renderFundHtmlWithoutHoldings(payload, code, baseUrl = "https://tanghuaizhe.github.io/qdii-direct-monitor/") {
  ...
  const fundStrategy = strategyLabel(fund.strategy);
  const band = feeBand(fee?.annualRate);
  ...
  const directDetail = salesDetailRow(rows, "direct");
  const agencyDetail = salesDetailRow(rows, "agency");
...
<div class="filters"><label class="search"><input id="fund-search" type="search" placeholder="搜索名称、代码、公司或拼音" aria-label="搜索基金"></label><div class="share-filters" aria-label="基金类型"><button class="filter-button active" type="button" data-strategy-filter="全部">全部</button>${strategies.map((strategy) => `<button class="filter-button" type="button" data-strategy-filter="${escapeHtml(strategy)}">${escapeHtml(strategyLabel(strategy))}</button>`).join("")}</div><div class="share-filters" aria-label="份额类别"><button class="filter-button active" type="button" data-share-filter="全部">全部</button>${shareClasses.map((share) => `<button class="filter-button" type="button" data-share-filter="${escapeHtml(share)}">${escapeHtml(share)} 类</button>`).join("")}</div><label class="toggle"><input id="purchasable-only" type="checkbox">仅看可购买</label><select id="fund-sort" class="sort" aria-label="排序方式"><option value="amount">额度从高到低</option><option value="fee">综合费率从低到高</option><option value="name">基金名称</option></select></div>
<div class="table-wrap"><table><thead><tr><th>基金</th><th><button class="filter-button column-sort" type="button" data-column-sort="agency" aria-label="按代销渠道每日额度排序">代销渠道 <span aria-hidden="true">↕</span></button></th><th><button class="filter-button column-sort" type="button" data-column-sort="direct" aria-label="按直销渠道每日额度排序">直销渠道 <span aria-hidden="true">↕</span></button></th><th><button class="filter-button column-sort" type="button" data-column-sort="fee" aria-label="按年综合费率排序">年综合费率 ⓘ <span aria-hidden="true">↕</span></button></th></tr></thead><tbody id="fund-body">${funds.map(([code, rows]) => fundTableRow(code, rows, feeByFund.get(code))).join("")}</tbody></table><div id="empty" class="empty">没有符合当前条件的基金</div></div></section>
...
(() => {
  ...
  const body = document.getElementById("fund-body"); const rows = [...body.querySelectorAll(".fund-row")]; const search = document.getElementById("fund-search"); const purchasable = document.getElementById("purchasable-only"); const sort = document.getElementById("fund-sort"); const count = document.getElementById("visible-count"); const empty = document.getElementById("empty"); const columnButtons = [...document.querySelectorAll("[data-column-sort]")]; let share = "全部"; let strategy = "全部"; let columnSort = null;
  function nullableNumber(value) { const number = Number(value); return Number.isFinite(number) && value !== "" ? number : null; }
  function compareNullableValues(left, right, direction) { if (left === null && right !== null) return 1; if (left !== null && right === null) return -1; if (left !== null && right !== null && left !== right) return (left - right) * direction; return 0; }
  function compareColumnRows(left, right) { const key = columnSort.key === "fee" ? "fee" : \`\${columnSort.key}Amount\`; const leftValue = nullableNumber(left.dataset[key]); const rightValue = nullableNumber(right.dataset[key]); if (leftValue === null && rightValue !== null) return 1; if (leftValue !== null && rightValue === null) return -1; if (leftValue !== null && rightValue !== null && leftValue !== rightValue) return (leftValue - rightValue) * columnSort.direction; return left.dataset.search.localeCompare(right.dataset.search, "zh-CN"); }
  function refresh() { const query = search.value.trim().toLowerCase().replace(/[^a-z0-9\\u3400-\\u9fff]+/g, ""); const visible = rows.filter((row) => (!query || row.dataset.search.includes(query)) && (share === "全部" || row.dataset.share === share) && (strategy === "全部" || row.dataset.strategy === strategy) && (!purchasable.checked || row.dataset.purchasable === "1")); const key = sort.value; visible.sort((a, b) => columnSort ? compareColumnRows(a, b) : key === "fee" ? compareNullableValues(nullableNumber(a.dataset.fee), nullableNumber(b.dataset.fee), 1) || a.dataset.search.localeCompare(b.dataset.search, "zh-CN") : key === "name" ? a.dataset.search.localeCompare(b.dataset.search, "zh-CN") : Number(b.dataset.amount) - Number(a.dataset.amount) || Number(a.dataset.fee || 0) - Number(b.dataset.fee || 0)); const visibleSet = new Set(visible); rows.forEach((row) => { row.hidden = !visibleSet.has(row); }); visible.forEach((row) => body.appendChild(row)); count.textContent = String(visible.length); empty.style.display = visible.length ? "none" : "block"; }
  function updateColumnButtons() { columnButtons.forEach((button) => { const active = columnSort?.key === button.dataset.columnSort; button.classList.toggle("active", active); button.querySelector("span").textContent = active ? (columnSort.direction === 1 ? "↑" : "↓") : "↕"; button.setAttribute("aria-sort", active ? (columnSort.direction === 1 ? "ascending" : "descending") : "none"); }); }
  columnButtons.forEach((button) => button.addEventListener("click", () => { const key = button.dataset.columnSort; if (columnSort?.key !== key) columnSort = { key, direction: 1 }; else if (columnSort.direction === 1) columnSort = { key, direction: -1 }; else { columnSort = null; sort.value = "amount"; } updateColumnButtons(); refresh(); }));
  document.querySelectorAll("[data-share-filter]").forEach((button) => button.addEventListener("click", () => { share = button.dataset.shareFilter; document.querySelectorAll("[data-share-filter]").forEach((item) => item.classList.toggle("active", item === button)); refresh(); })); document.querySelectorAll("[data-strategy-filter]").forEach((button) => button.addEventListener("click", () => { strategy = button.dataset.strategyFilter; document.querySelectorAll("[data-strategy-filter]").forEach((item) => item.classList.toggle("active", item === button)); refresh(); })); search.addEventListener("input", refresh); purchasable.addEventListener("change", refresh); sort.addEventListener("change", () => { columnSort = null; updateColumnButtons(); refresh(); }); updateColumnButtons(); refresh();
})();
</script></body></html>\n`;
}

module.exports = { aggregateStatus, amountLabel, channelLabel, escapeHtml, feeBand, feeLabel, purchaseSummaryHtml, rankPurchasableFunds, reliabilityReasonLabel, renderFundHtml, renderHtml, statusLabel, strategyLabel };
