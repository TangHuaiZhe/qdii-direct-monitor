...
async function collectEastmoney(fund, context) {
  ...
  try {
    ...
    return {
      fundCode: fund.code, fundName: fund.name, manager: fund.manager, index: fund.index || "nasdaq100", currency: amount?.currency || fund.currency || "CNY", shareClass: fund.shareClass || "",
    ...
    return {
      fundCode: fund.code, fundName: fund.name, manager: fund.manager, index: fund.index || "nasdaq100", currency: fund.currency || "CNY", shareClass: fund.shareClass || "",
...
function collectManual(fund, observedAt, warnings) {
  ...
  return (fund.manualChannels || []).flatMap((item) => {
    ...
    return [{ fundCode: fund.code, fundName: fund.name, manager: fund.manager, index: fund.index || "nasdaq100", currency: item.currency || fund.currency || "CNY", shareClass: fund.shareClass || "",
